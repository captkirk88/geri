...
append_glyph_preloaded :: proc(batch: ^Batch2D, f: ^Font, g: Cached_Glyph, x_pos, y_pos: f32, color: [4]f32, vp: linalg.Matrix4f32) {
...
append_glyph :: proc(batch: ^Batch2D, f: ^Font, r: rune, x_pos, y_pos: f32, color: [4]f32, vp: linalg.Matrix4f32) -> f32 {
	g := get_glyph(f, r)
...
parse_color_to_rgb :: proc(name: string) -> [3]f32 {
	...
	case "magenta", "purple": return {1.0, 0.0, 1.0}
...
parse_bbcode_spans :: proc(text: string, default_color: [4]f32 = {1, 1, 1, 1}, allocator := context.allocator) -> []Text_Span {
	...
	get_current_style :: proc(
		...
		bg_opacity_stack: ^[dynamic]f32,
		has_bg_stack: ^[dynamic]bool,
	) -> (color: [4]f32, bg_color: [4]f32, has_bg: bool) {
	...
	for i < n {
		if text[i] == '[' {
			if i > start_idx {
				c, bg_c, hb := get_current_style(&color_stack, &opacity_stack, &bg_color_stack, &bg_opacity_stack, &has_bg_stack)
				append(&spans, Text_Span{
					text     = text[start_idx:i],
					color    = c,
					bg_color = bg_c,
					has_bg   = hb,
				})
...
append_char :: proc(batch: ^Batch2D, ch: u8, x_pos, y_pos: f32, scale: f32, color: [4]f32, vp: linalg.Matrix4f32) {
	c_idx := int(ch)
...
draw_text_bbcode :: proc(
	...
	for span in spans {
		for i in 0 ..< len(span.text) {
			...
			if span.has_bg {
				append_quad(batch, cursor_x, cursor_y, cursor_x + 8.0 * scale, cursor_y + 8.0 * scale, span.bg_color, vp)
			}
