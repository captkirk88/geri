...
sdl_to_keycode :: proc(key: sdl3.Keycode) -> KeyCode {
	...
	case sdl3.K_0: return .Num0
	case sdl3.K_1: return .Num1
	case sdl3.K_2: return .Num2
	case sdl3.K_3: return .Num3
	case sdl3.K_4: return .Num4
	case sdl3.K_5: return .Num5
	case sdl3.K_6: return .Num6
	case sdl3.K_7: return .Num7
	case sdl3.K_8: return .Num8
	case sdl3.K_9: return .Num9
	case sdl3.K_SPACE: return .Space
	case sdl3.K_RETURN: return .Enter
...
sdl_to_buttoncode :: proc(btn: u8) -> ButtonCode {
	switch btn {
	case 1: return .Left
...
sdl_to_gamepad_button :: proc(btn: sdl3.GamepadButton) -> GamepadButton {
	...
	case .LEFT_STICK:     return .LeftStick
	case .RIGHT_STICK:    return .RightStick
...
sdl_to_gamepad_axis :: proc(axis: sdl3.GamepadAxis) -> GamepadAxis {
	...
	case .LEFT_TRIGGER:   return .TriggerLeft
	case .RIGHT_TRIGGER:  return .TriggerRight
	}
	return .None
}
...
input_update_system :: proc(
	...
	clear(&state.keys_released)
	clear(&state.buttons_pressed)
	clear(&state.buttons_released)
	...
	for ev in events.events {
		...
		case .KEY_DOWN:
			...
			if kc != .None {
				if !state.keys_down[kc] {
					state.keys_pressed[kc] = true
				}
				state.keys_down[kc] = true
			}
		case .KEY_UP:
			kc := sdl_to_keycode(ev.key.key)
			if kc != .None {
				state.keys_down[kc] = false
				state.keys_released[kc] = true
			}
		case .MOUSE_MOTION:
			state.mouse_position = {ev.motion.x, ev.motion.y}
			if state.buttons_down[.Left] {
		...
		case .MOUSE_BUTTON_DOWN:
			...
			if btn != .None {
				...
				state.buttons_pressed[btn] = true
				state.buttons_down[btn] = true
				if btn == .Left {
		...
		case .MOUSE_BUTTON_UP:
			...
			if btn != .None {
				state.mouse_position = {ev.button.x, ev.button.y}
				state.buttons_down[btn] = false
				state.buttons_released[btn] = true
		...
		case .MOUSE_WHEEL:
			...
			scale_delta := ev.wheel.y * 0.1
			state.pinch_scale = clamp(state.pinch_scale + scale_delta, 0.1, 10.0)

		case .PINCH_BEGIN:
			state.gestures_active[.Pinch] = true
		case .PINCH_UPDATE:
			state.gestures_active[.Pinch] = true
			state.pinch_scale = clamp(state.pinch_scale * ev.pinch.scale, 0.1, 10.0)
		case .PINCH_END:
			// reset or keep scale
