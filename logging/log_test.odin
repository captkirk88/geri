...
test_color_processing :: proc(t: ^testing.T) {
	...
	// Nested colors
	res3 := bbcode.process_colors("[c=ff0000]red [c=00ff00]green[/c] red[/c]", true, context.allocator)
	defer delete(res3)
	testing.expect_value(t, res3, "\x1b[0m\x1b[38;2;255;0;0mred \x1b[0m\x1b[38;2;0;255;0mgreen\x1b[0m\x1b[38;2;255;0;0m red\x1b[0m")

	// Predefined colors
	res4 := bbcode.process_colors("hello [c=red]red[/c] and [c=blue]blue[/c]", true, context.allocator)
	defer delete(res4)
	...
	defer delete(res5)
	testing.expect_value(t, res5, "hello \x1b[0m\x1b[1mbold\x1b[0m and \x1b[0m\x1b[3mitalic\x1b[0m")

	// Nested Bold and Italic
	res6 := bbcode.process_colors("[b]bold [i]italic[/i] bold[/b]", true, context.allocator)
	defer delete(res6)
	testing.expect_value(t, res6, "\x1b[0m\x1b[1mbold \x1b[0m\x1b[1m\x1b[3mitalic\x1b[0m\x1b[1m bold\x1b[0m")
	...
	res8 := bbcode.process_colors("TEXTURE: [ID 1] Texture loaded successfully", true, context.allocator)
	defer delete(res8)
	...
	res10 := bbcode.process_colors("[u]underlined[/u] and [s]striked[/s] and [bg=red]red bg[/bg]", true, context.allocator)
	defer delete(res10)
	testing.expect_value(t, res10, "\x1b[0m\x1b[4munderlined\x1b[0m and \x1b[0m\x1b[9mstriked\x1b[0m and \x1b[0m\x1b[41mred bg\x1b[0m")
...
test_rolling_file_on_launch :: proc(t: ^testing.T) {
	...
	add_output(create_rolling_file_output(.Debug, dir, "test_launch", .On_Launch, DEFAULT_FILE_FORMAT))
	...
	testing.expect(t, !strings.contains(content, "\x1b["), "File should not contain ANSI escape codes")
}
...
test_rolling_file_on_day :: proc(t: ^testing.T) {
	...
	add_output(create_rolling_file_output(.Debug, dir, "test_day", .On_Calendar_Day, DEFAULT_FILE_FORMAT))
...
test_proxy_output :: proc(t: ^testing.T) {
	...
	console_out := create_console_output(.Debug, Log_Format{enable_color = false, options = log.Default_Console_Logger_Opts}, context.allocator)
	proxy_out := create_proxy_output(.Info, console_out)
...
test_bbcode_validation :: proc(t: ^testing.T) {
	...
	_, err2, ok2 := bbcode.validate_and_format_bbcode("[b]unclosed", true, false, allocator = context.allocator)
	...
	// 3. Unknown tag error
	_, err3, ok3 := bbcode.validate_and_format_bbcode("[unknown]text", true, false, allocator = context.allocator)
	defer if !ok3 { delete(err3) }
	testing.expect(t, !ok3, "Expected unknown tag to fail")

	// 4. Unescaped [ error
	_, err4, ok4 := bbcode.validate_and_format_bbcode("unescaped [ bracket", true, false, allocator = context.allocator)
	defer if !ok4 { delete(err4) }
	testing.expect(t, !ok4, "Expected unescaped [ to fail")

	// 5. Unescaped ] error
	_, err5, ok5 := bbcode.validate_and_format_bbcode("unescaped ] bracket", true, false, allocator = context.allocator)
	defer if !ok5 { delete(err5) }
	testing.expect(t, !ok5, "Expected unescaped ] to fail")

	// 6. Escaped bracket sequence
	res6, err6, ok6 := bbcode.validate_and_format_bbcode("escaped \\[b\\] and \\[i\\] and [b]real bold[/b]", true, false, allocator = context.allocator)
	defer delete(res6)
	...
	res7, err7, ok7 := bbcode.validate_and_format_bbcode("double [[level]] and [b]real bold[/b]", true, true, "", "", "info", "", "", allocator = context.allocator)
	defer delete(res7)
...
test_time_formatting :: proc(t: ^testing.T) {
	// None
	fmt_none := Log_Format{time_format = .None}
	t_none := get_time_string(fmt_none, context.allocator)
	defer delete(t_none)
	testing.expect_value(t, t_none, "")

	// Long UTC (e.g. YYYY-MM-DD HH:MM:SS)
	fmt_long_utc := Log_Format{time_format = .Long_UTC}
	t_long_utc := get_time_string(fmt_long_utc, context.allocator)
	defer delete(t_long_utc)
	testing.expect(t, len(t_long_utc) == len("[YYYY-MM-DD HH:MM:SS] "), "Long UTC time string length should be standard")

	// Short UTC (e.g. HH:MM:SS)
	fmt_short_utc := Log_Format{time_format = .Short_UTC}
	t_short_utc := get_time_string(fmt_short_utc, context.allocator)
	defer delete(t_short_utc)
	testing.expect(t, len(t_short_utc) == len("[HH:MM:SS] "), "Short UTC time string length should be standard")

	// Custom (UTC+2 = +120 minutes)
	fmt_custom := Log_Format{time_format = .Custom, timezone_offset = 120}
	t_custom := get_time_string(fmt_custom, context.allocator)
	defer delete(t_custom)
	testing.expect(t, len(t_custom) == len("[YYYY-MM-DD HH:MM:SS] "), "Custom time string length should be standard")
}
...
test_dynamic_tag_registration :: proc(t: ^testing.T) {
	...
	tag_prefix :: proc(args: string, ctx: bbcode.Tag_Func_Context, allocator: runtime.Allocator) -> (result: string, err_msg: string, ok: bool) {
...
test_builtin_upper_lower :: proc(t: ^testing.T) {
	...
	level_str := get_level_string(.Info, Log_Format{template = "custom", enable_color = true}, context.allocator)
	...
	testing.expect_value(t, res_temp, "\x1b[0m\x1b[4m\x1b[0m\x1b[1m\x1b[4m\x1b[1;37minfo\x1b[0m\x1b[0m\x1b[4m\x1b[0mHello, World!")

	// Test default layout template variable (has colon, space after reset)
	level_str_default := get_level_string(.Info, Log_Format{template = "", enable_color = true}, context.allocator)
	...
	testing.expect_value(t, res_default, "\x1b[0m\x1b[4m\x1b[0m\x1b[1m\x1b[4m\x1b[1;37minfo:\x1b[0m \x1b[0m\x1b[4m\x1b[0mHello, World!")
}
...
test_tag_suppression :: proc(t: ^testing.T) {
	...
	line := format_log_line(format_suppressed, .Info, "Hello, Suppressed!", {}, loc, {.Location, .Time}, context.allocator)
	defer delete(line)
...
test_thread_tag_suppression :: proc(t: ^testing.T) {
	...
	line := format_log_line(format, .Info, "Hello, Thread Suppressed!", {}, loc, {}, context.allocator)
	defer delete(line)
...
test_output_struct_tag_suppression :: proc(t: ^testing.T) {
	out := create_console_output(.Info, Log_Format{
		template     = "[[time]][[location]][level]: [message]",
		enable_color = false,
		time_format  = .Long,
	}, context.allocator)
	...
	line := format_log_line(c_data.format, .Info, "Hello, Output Struct Suppressed!", {}, loc, out.suppressed_tags, context.allocator)
	defer delete(line)
...
test_user_template_conditional :: proc(t: ^testing.T) {
	...
	testing.expect(t, strings.contains(line, "Hello debug message"), "Expected message to be in the output")
	
	// Yellow ANSI code is ANSI_YELLOW ("\x1b[33m").
	// Gray ANSI code is ANSI_GRAY ("\x1b[90m").
	testing.expect(t, strings.contains(line, "\x1b[33mHello debug message"), "Expected debug message to be formatted in yellow")
	testing.expect(t, !strings.contains(line, "\x1b[90mHello debug message"), "Did not expect debug message to be formatted in gray")
}
